...
/**
 * 
 * A wrapper-class for {@link Classifier} of the MOA library.
 * 
 * @author Robert Meusel (robert@dwslab.de)
 * 
 */
public class AnthOnlineClassifier {

  private Classifier learner;
  private Queue<Instance> learningQueue;
  private int dimension;
  private Instances instances;
  protected long unclassifiableItems;
  protected long classifiedItems;
  private double[] replaceMissingValues;
  ...
  private String stringPattern = "[a-z]*";
  private String numReplacementString = "[NUMBER]";
  private int hashTrickSize;
  private int batchLearningSize;
   ...
   *            the {@link AnthURL}
   */
  public void classifyUrl(AnthURL aurl) {
  ...
  /**
   * Instanciates the {@link AnthOnlineClassifier} class which wrapes all
   * necessary information to do online classification (training and
   * classification itself) based on {@link AnthURL}.
   */
  public AnthOnlineClassifier(String classifierName,
      String classifierOptions, int hashTrickSize, int batchLearnSize) {
   ...
   */
  public void pushFeedback(AnthURL aurl) {
    Instance inst = convert(aurl);
  ...
  public void initialize(List<AnthURL> list) {
    System.out.println("Initializing learner with " + list.size()
    + " Urls.");
   ...
   */
  private Set<String> tokenizer(String string) {
