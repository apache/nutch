...
public class TestSuffixURLFilter {
  ...
  public void testModeReject() {
    ...
    for (int i = 0; i < urls.length; i++) {
      assertSame(urlsModeReject[i], filter.filter(urls[i]));
    }
  }
  ...
  public void testModeAcceptIgnoreCase() {
    ...
    filter.setModeAccept(true);
    for (int i = 0; i < urls.length; i++) {
      assertSame(urlsModeAcceptIgnoreCase[i], filter.filter(urls[i]));
    }
  }
  ...
  public void testModeRejectIgnoreCase() {
    ...
    filter.setModeAccept(false);
    for (int i = 0; i < urls.length; i++) {
      assertSame(urlsModeRejectIgnoreCase[i], filter.filter(urls[i]));
    }
  }
  ...
  public void testModeAcceptAndNonPathFilter() {
    ...
    for (int i = 0; i < urls.length; i++) {
      assertSame(urlsModeAcceptAndNonPathFilter[i], filter.filter(urls[i]));
    }
  }
  ...
  public void testModeAcceptAndPathFilter() {
    ...
    for (int i = 0; i < urls.length; i++) {
      assertSame(urlsModeAcceptAndPathFilter[i], filter.filter(urls[i]));
    }
  }
