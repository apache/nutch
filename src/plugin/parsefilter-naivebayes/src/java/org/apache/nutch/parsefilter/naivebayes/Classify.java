...
public class Classify {
	...
	public static String classify(String line) throws IOException{
		...
		if(!ismodel){
			    ...
			    BufferedReader bufferedReader=new BufferedReader(new InputStreamReader(fs.open(new Path("naivebayes-model"))));
		           ...
		           wordfreq_ir=unflattenToHashmap(bufferedReader.readLine());
		            ...
		            ismodel=true;
		               ...
		               for(String word:linearray){
		            	   ...
		            	   prob_ir+=Math.log(wordfreq_ir.get(word))+1-Math.log(numwords_ir+uniquewords_size);
		            	   else
		            		   prob_ir+=1-Math.log(numwords_ir+uniquewords_size);
		            	   
		            	   if(wordfreq_r.containsKey(word))
			            	   prob_r+=Math.log(wordfreq_r.get(word))+1-Math.log(numwords_r+uniquewords_size);
			            	   else
