...
public class HBaseIndexWriter implements IndexWriter {
  ...
  public void open(Configuration conf) throws IOException {
    ...
    batchSize = conf.getInt(HBaseConstants.HBASE_COMMIT_SIZE, HBaseConstants.DEFAULT_COMMIT_SIZE);
    zkQuorum = config.get(HBaseConstants.ZOOKEEPER_QUORUM, HBaseConstants.DEFAULT_ZOOKEEPER_QUORUM);
    zkClientPort = config.getInt(HBaseConstants.ZOOKEEPER_CLIENT_PORT, HBaseConstants.DEFAULT_ZOOKEEPER_CLIENT_PORT);
    ...
    if(!hBaseAdmin.tableExists(hbaseMapping.getTableName())) {
      String msg = "Table '" + hbaseMapping.getTableName() + "' doesn't exist in hbase";
      LOG.error(msg);
  ...
  public void write(NutchDocument doc) throws IOException {
    ...
    while(entries.hasNext()) {
      ...
      put.add(Bytes.toBytes(hbaseMapping.getFamily(entry.getKey())), Bytes.toBytes(entry.getKey()), Bytes.toBytes(entry.getValue().toString()));
      puts.add(put);
    }
  ...
  public String describe() {
    ...
    sb.append("\t").append(HBaseConstants.ZOOKEEPER_QUORUM).append(" : Zookeeper quorum (default \"localhost\")\n");
    sb.append("\t").append(HBaseConstants.ZOOKEEPER_CLIENT_PORT).append(" : Zookeeper client port (default 2181)\n");
    sb.append("\t").append(HBaseConstants.COMMIT_SIZE).append(" : buffer size when sending to Hbase (default 250)");
    return sb.toString();
  }
